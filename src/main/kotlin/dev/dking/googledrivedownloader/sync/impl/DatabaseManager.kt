...
internal class DatabaseManager(databasePath: Path) : AutoCloseable {
     ...
     */
    fun createSyncRun(startedAt: Instant, startPageToken: String?): Long {
        val sql = """
        ...
        connection.prepareStatement("SELECT last_insert_rowid()").use { stmt ->
            ...
            return if (rs.next()) rs.getLong(1) else throw IllegalStateException("Failed to get sync run ID")
        }
    }
     ...
     */
    fun updateSyncRunProgress(syncRunId: Long, filesProcessed: Int, bytesDownloaded: Long) {
        val sql = """
            UPDATE sync_runs
     ...
     */
    fun completeSyncRun(syncRunId: Long, completedAt: Instant, status: String, errorMessage: String? = null) {
        val sql = """
            UPDATE sync_runs
     ...
     */
    fun saveChangeToken(token: String, updatedAt: Instant) {
        val sql = """
