...
class SyncEngineImpl(
    ...
    override fun initialSync(): Flow<SyncEvent> = channelFlow {
        ...
        try {
            DatabaseManager(databasePath).use { db ->
                val fileOps = FileOperations(config.downloadDirectory, driveClient, config.exportFormats, db)
                ...
                val filesResult = driveClient.listAllFiles(
                    setOf(
                        FileField.ID,
                        FileField.NAME,
                        FileField.MIME_TYPE,
                ...
                val downloadResult = downloadPendingFiles(
                    db = db,
                    fileOps = fileOps,
                    ...
                    onCompleted = { fileId, fileName, bytes ->
                        ...
                        send(SyncEvent.Progress(filesProcessed, driveFiles.size, bytesDownloaded))
                    },
                ...
                send(SyncEvent.Completed(filesProcessed, bytesDownloaded, failedFiles, duration))
            }
        } catch (e: Exception) {
    ...
    override fun incrementalSync(): Flow<SyncEvent> = channelFlow {
        val startTime = Instant.now()
        var filesProcessed = 0
        var bytesDownloaded = 0L
        var failedFiles = 0
                    ...
                    if (change.removed || change.file == null) {
                        ...
                        if (config.deleteRemovedFiles) {
                            ...
                            if (existingFile?.localPath != null) {
                                val localPath = config.downloadDirectory.resolve(existingFile.localPath)
                ...
                if (pendingCount > 0) {
                    ...
                    downloadPendingFiles(
                        ...
                        onCompleted = { fileId, fileName, bytes ->
                            ...
                            bytesDownloaded += bytes
                            send(SyncEvent.FileCompleted(fileId, fileName))
                            send(SyncEvent.Progress(filesProcessed, pendingCount, bytesDownloaded))
                        },
                        onFailed = { fileId, fileName, error ->
                            failedFiles++
                            send(SyncEvent.FileFailed(fileId, fileName, error))
    ...
    override fun resumeSync(): Flow<SyncEvent> = channelFlow {
        val startTime = Instant.now()
        var filesProcessed = 0
        var bytesDownloaded = 0L
        var failedFiles = 0
        ...
        try {
            DatabaseManager(databasePath).use { db ->
                ...
                if (lastRun != null && (lastRun.status == "running" || lastRun.status == "interrupted")) {
                    ...
                    val pendingFiles = db.getFilesByStatuses(
                    ...
                    downloadPendingFiles(
                        ...
                        onCompleted = { fileId, fileName, bytes ->
                            ...
                            bytesDownloaded += bytes
                            send(SyncEvent.FileCompleted(fileId, fileName))
                            send(SyncEvent.Progress(filesProcessed, pendingFiles.size, bytesDownloaded))
                        },
                        onFailed = { fileId, fileName, error ->
                            failedFiles++
                            send(SyncEvent.FileFailed(fileId, fileName, error))
                    ...
                    db.updateSyncRunProgress(lastRun.id, lastRun.filesProcessed + filesProcessed, lastRun.bytesDownloaded + bytesDownloaded)
        ...
        for (folder in folders) {
            ...
            try {
                ...
                if (result.isSuccess) {
                    ...
                    db.updateFileStatus(folder.id, FileRecord.SyncStatus.ERROR, errorMessage = error)
            ...
            val jobs = files.map { file ->
                async {
                    ...
                    try {
                        ...
                        if (result.isSuccess) {
                            db.updateFileStatus(file.id, FileRecord.SyncStatus.COMPLETE, Instant.now())
                            onCompleted(file.id, file.name, file.size ?: lastBytes)
                        } else {
                            val error = result.exceptionOrNull()?.message ?: "Unknown error"
                            db.updateFileStatus(file.id, FileRecord.SyncStatus.ERROR, errorMessage = error)
