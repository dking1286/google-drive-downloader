    ...
    repeat(retryAttempts) { attempt ->
      try {
        ...
        if (attempt < retryAttempts - 1) {
          ...
          logger.warn {
            "Transient error on attempt ${attempt + 1}, retrying after ${delayMs}ms: ${e.message}"
          }
  ...
  private fun isTransientError(error: Exception): Boolean {
    return when (error) {
      ...
      is GoogleJsonResponseException -> {
        ...
        logger.debug {
          "GoogleJsonResponseException status: $statusCode, message: ${error.message}"
        }
        val isTransient =
          when (statusCode) {
            429 -> true // Rate limit
        ...
        val classification = if (isTransient) "transient" else "permanent"
        logger.debug {
          "Classified GoogleJsonResponseException (status $statusCode) as $classification"
        }
        isTransient
      }
      ...
      is IOException -> {
        ...
        logger.debug { "Classified unknown exception $exceptionName as permanent (default)" }
        false
      }
    }
  }
        ...
        when (error.statusCode) {
          ...
          429, 500, 502, 503, 504 ->
            TransientApiException(
              "Transient API error: ${error.message}",
              error,
            )
