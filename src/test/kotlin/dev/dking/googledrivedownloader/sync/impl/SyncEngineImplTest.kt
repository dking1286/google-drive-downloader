...
import dev.dking.googledrivedownloader.api.*
...
class SyncEngineImplTest {
    ...
    fun setup() {
        ...
        config = SyncEngineConfig(
            ...
            exportFormats = mapOf(
                "application/vnd.google-apps.document" to "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    ...
    @Test
    fun `getSyncStatus returns empty status when no sync has been performed`() = runTest {
    ...
    @Test
    fun `getSyncStatus returns correct status after initial sync`() = runTest {
        // Setup mock responses
        val pageToken = "token123"
        val testFile = DriveFile(
            id = "file1",
            name = "test.txt",
            mimeType = "text/plain",
            parentId = null,
    ...
    @Test
    fun `getFailedFiles returns empty list when no failures`() = runTest {
    ...
    @Test
    fun `getFailedFiles returns files with error status`() = runTest {
        ...
        coEvery { driveClient.getStartPageToken() } returns Result.success(pageToken)
        coEvery { driveClient.listAllFiles(any()) } returns Result.success(listOf(testFile))
        coEvery { driveClient.downloadFile(any(), any(), any()) } returns Result.failure(Exception("Download failed"))
    ...
    @Test
    fun `initialSync successfully downloads files`() = runTest {
        val pageToken = "token123"
        val testFile = DriveFile(
            id = "file1",
            name = "test.txt",
    ...
    @Test
    fun `initialSync handles folders correctly`() = runTest {
        val pageToken = "token123"
        val testFolder = DriveFile(
            id = "folder1",
            name = "MyFolder",
        ...
        coEvery { driveClient.getStartPageToken() } returns Result.success(pageToken)
        coEvery { driveClient.listAllFiles(any()) } returns Result.success(listOf(testFolder, testFile))
        coEvery { driveClient.downloadFile(any(), any(), any()) } answers {
            val outputPath = secondArg<java.nio.file.Path>()
            java.nio.file.Files.createDirectories(outputPath.parent)
            java.nio.file.Files.write(outputPath, ByteArray(100))
    ...
    @Test
    fun `initialSync handles Google Workspace files`() = runTest {
        val pageToken = "token123"
        val testDoc = DriveFile(
            id = "doc1",
            name = "MyDocument",
    ...
    @Test
    fun `initialSync fails when cannot get start page token`() = runTest {
    ...
    @Test
    fun `initialSync fails when cannot list files`() = runTest {
    ...
    @Test
    fun `initialSync continues after individual file failures`() = runTest {
        val pageToken = "token123"
        val testFile1 = DriveFile(
            id = "file1",
            name = "success.txt",
            mimeType = "text/plain",
            parentId = null,
        ...
        coEvery { driveClient.getStartPageToken() } returns Result.success(pageToken)
        coEvery { driveClient.listAllFiles(any()) } returns Result.success(listOf(testFile1, testFile2))
        ...
        coEvery { driveClient.downloadFile("file2", any(), any()) } returns Result.failure(Exception("Download failed"))
    ...
    @Test
    fun `incrementalSync fails when no change token exists`() = runTest {
    ...
    @Test
    fun `incrementalSync detects and downloads new files`() = runTest {
    ...
    @Test
    fun `incrementalSync detects modified files`() = runTest {
        // Initial sync with a file
        val pageToken = "token123"
        val originalFile = DriveFile(
            id = "file1",
            name = "test.txt",
            mimeType = "text/plain",
            parentId = null,
        ...
        val modifiedFile = originalFile.copy(
    ...
    @Test
    fun `incrementalSync handles removed files when deleteRemovedFiles is false`() = runTest {
        // Initial sync with a file
        val pageToken = "token123"
        val originalFile = DriveFile(
            id = "file1",
    ...
    @Test
    fun `incrementalSync deletes removed files when deleteRemovedFiles is true`() = runTest {
        // Update config
    ...
    @Test
    fun `incrementalSync handles empty change list`() = runTest {
        // Initial sync
        val pageToken = "token123"
        coEvery { driveClient.getStartPageToken() } returns Result.success(pageToken)
        coEvery { driveClient.listAllFiles(any()) } returns Result.success(emptyList())
    ...
    @Test
    fun `incrementalSync fails when cannot list changes`() = runTest {
        // Initial sync
        val pageToken = "token123"
        coEvery { driveClient.getStartPageToken() } returns Result.success(pageToken)
        coEvery { driveClient.listAllFiles(any()) } returns Result.success(emptyList())
    ...
    @Test
    fun `resumeSync performs incremental sync when no interrupted sync exists`() = runTest {
        // Initial sync
        val pageToken = "token123"
        coEvery { driveClient.getStartPageToken() } returns Result.success(pageToken)
        coEvery { driveClient.listAllFiles(any()) } returns Result.success(emptyList())
    ...
    @Test
    fun `resumeSync continues interrupted sync when pending files exist`() = runTest {
        ...
        coEvery { driveClient.listChanges(any()) } returns Result.success(
